...
/**
 * 头部状态栏的设置
 * Created by SunySAan on 2016/11/4.
 */
public class ToolBarStatusUtils {
...
///////////////////////////////////////////////////////////////////////////////////////////////
     ...
     * @return
     */
    public static int getStatusBarH(Activity m) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            Class<?> c = null;
            Object obj = null;
            Field field = null;
     ...
     * 设置状态栏颜色
     * @param mActivity
     * @param view 预先设置的view代替状态栏
     * @param color
     * @param isTop 是否延伸到状态栏 true为延伸，false为不延伸
     */
    public static void setColors(Activity mActivity, View view, int color, boolean isTop) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            if (isTop){
                color = R.color.status_transparent;
            }
            setTranslucentStatus(mActivity, true);
            if (ToolBarStatusUtils.statusHeight > 0) {
                ViewGroup.LayoutParams params = view.getLayoutParams();
                params.height = ToolBarStatusUtils.statusHeight;
                params.width = ViewGroup.LayoutParams.MATCH_PARENT;
                view.setLayoutParams(params);
            } else {
                int i = getStatusBarH(mActivity);
                if (i > 0) {
                    ToolBarStatusUtils.statusHeight = i;
                    ViewGroup.LayoutParams params = view.getLayoutParams();
                    params.height = ToolBarStatusUtils.statusHeight;
                    params.width = ViewGroup.LayoutParams.MATCH_PARENT;
                    view.setLayoutParams(params);
